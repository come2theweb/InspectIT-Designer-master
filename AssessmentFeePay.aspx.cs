...
namespace InspectIT
{
    public partial class AssessmentFeePay : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (theSqlDataReader.HasRows)
                {
                    while (theSqlDataReader.Read())
                    {
                        ...
                        {
                            AssessmentFeeAmount = Convert.ToDecimal(theSqlDataReader["Amount"].ToString());
                        }
                        AssessmentFee.Text = AssessmentFeeAmount.ToString();
                    }
                }
                if (theSqlDataReader.IsClosed) theSqlDataReader.Close();
                DLdb.RS.Close();
                ...
                decimal vatC = AssessmentFeeAmount * vat;
                VATCOCPurchase.Text = vatC.ToString();
                decimal totalAmount = AssessmentFeeAmount + vatC;
        ...
        protected void btn_buy_Click(object sender, EventArgs e)
        {
            ...
            if (AssessType.SelectedValue=="" || locations.SelectedValue==""|| !CheckBox1.Checked)
            {
                errormsg.InnerHtml = "Please make sure you have all relevant fields selected and checked";
                errormsg.Visible = true;
            }
            else
            {
                ...
                DLdb.RS.Open();
                DLdb.SQLST.CommandText = "INSERT INTO Orders (WantCard,AssessmentRules,AssessmentLocation,AssessmentType,UserID,Description,TotalNoItems,SubTotal,Vat,Delivery,Total,Method,isPaid) values (@WantCard,@AssessmentRules,@AssessmentLocation,@AssessmentType,@UserID,@Description,@TotalNoItems,@SubTotal,@Vat,@Delivery,@Total,@Method,@isPaid); Select Scope_Identity() as OrderID";
                DLdb.SQLST.Parameters.AddWithValue("UserID", Session["IIT_UID"].ToString());
                DLdb.SQLST.Parameters.AddWithValue("Description", "Assessment Fee");
                DLdb.SQLST.Parameters.AddWithValue("TotalNoItems", "1");
                DLdb.SQLST.Parameters.AddWithValue("SubTotal", AssessmentFee.Text.ToString());
                ...
                DLdb.SQLST.Parameters.AddWithValue("AssessmentType", AssessType.SelectedValue.ToString());
                DLdb.SQLST.Parameters.AddWithValue("AssessmentLocation", locations.SelectedValue.ToString());
                DLdb.SQLST.Parameters.AddWithValue("WantCard", CheckBox2.Checked ? 1 : 0);
                DLdb.SQLST.Parameters.AddWithValue("AssessmentRules", CheckBox1.Checked ? 1:0);
                DLdb.SQLST.CommandType = CommandType.Text;
                DLdb.SQLST.Connection = DLdb.RS;
                SqlDataReader theSqlDataReader = DLdb.SQLST.ExecuteReader();
                ...
                DLdb.SQLST.Parameters.AddWithValue("UserID", Session["IIT_UID"].ToString());
                DLdb.SQLST.Parameters.AddWithValue("OrderID", OrderID.ToString());
                ...
                DLdb.SQLST.Parameters.AddWithValue("isPaid", "0");
                DLdb.SQLST.Parameters.AddWithValue("NewCard", CheckBox2.Checked ? 1 : 0);
                DLdb.SQLST.CommandType = CommandType.Text;
                DLdb.SQLST.Connection = DLdb.RS;
                theSqlDataReader = DLdb.SQLST.ExecuteReader();
        ...
        protected void Group1_CheckedChanged(Object sender, EventArgs e)
        {
            ...
            if (theSqlDataReader.HasRows)
            {
                while (theSqlDataReader.Read())
                {
                    ...
                    {
                        AssessmentCost = Convert.ToDecimal(theSqlDataReader["Amount"].ToString());
                    }
                    AssessmentFee.Text  = AssessmentCost.ToString();
            ...
            decimal vatC = (AssessmentCost + CardCost) * vat;
            VATCOCPurchase.Text = vatC.ToString();
            decimal totalAmount = CardCost + vatC + AssessmentCost;
